...
def export_dense_mlp_to_onnx(
    ...
    initializers.append(
        helper.make_tensor("W1", TensorProto.FLOAT, [obs_dim, 256], w1.flatten())
    )
    initializers.append(
    ...
    nodes.append(
        helper.make_node("Gemm", ["h1_act", "W2", "B2"], ["h2"], alpha=1.0, beta=1.0)
    )
    ...
    initializers.append(
        helper.make_tensor("W3", TensorProto.FLOAT, [256, act_dim], w3.flatten())
    )
    initializers.append(
...
def main():
    ...
    export_dense_mlp_to_onnx(weights_dict, args.obs_dim, args.act_dim, args.output_path)
